...
namespace IS_Test
{
    public partial class PUT_usergroupidprivilege : BaseForm
    {
        ...
        public PUT_usergroupidprivilege()
        {
            InitializeComponent();
        ...
        {
            string URI = string.Format("http://{0}:{1}/integrationserver/userGroup/{2}/privilege?global", Main.SERVER, Main.PORT,id);
        ...
        private string CreateXML()
        {
            ...
            if (Privileges.privilegeGroups[0].id != null)
            {
                sb.Append(string.Format("<id>{0}</id>", Privileges.privilegeGroups[0].id.Trim()));
            }
